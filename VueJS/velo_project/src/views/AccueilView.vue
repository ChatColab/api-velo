<script setup>
import { ref, onMounted } from 'vue'
import ComposantGeo from "@/components/ComposantGeo.vue"
import axios from 'axios'

const currentCoordinates = ref([49.8652904994832, 2.3786318])
const parks = ref([])

async function fetchAndDisplayMarkers(bounds) {
    const token = '';

    const minLatitude = bounds.getSouthWest().lat;
    const maxLatitude = bounds.getNorthEast().lat;
    const minLongitude = bounds.getSouthWest().lng;
    const maxLongitude = bounds.getNorthEast().lng;

    let apiUrl = `http://localhost:1337/api/velos?filters[x][$gte]=${minLongitude}&filters[x][$lte]=${maxLongitude}&filters[y][$gte]=${minLatitude}&filters[y][$lte]=${maxLatitude}`;

    // Get the first page to know the total number of pages.
    let response = await axios.get(apiUrl, { headers: { 'Authorization': `Bearer ${token}` } });
    let pageCount = response.data.meta.pagination.pageCount;
    parks.value = response.data.data;

    // Fetch all other pages.
    for (let page = 2; page <= pageCount; page++) {
        let pagedUrl = `${apiUrl}&pagination[page]=${page}`;
        response = await axios.get(pagedUrl, { headers: { 'Authorization': `Bearer ${token}` } });
        parks.value = parks.value.concat(response.data.data);
    }
}

</script>

<template>
    <div>
        <ComposantGeo :parks="parks" @updateMarkers="fetchAndDisplayMarkers"/>
    </div>
</template>
